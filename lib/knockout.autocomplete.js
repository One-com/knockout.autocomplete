...
(function () {
    ...
    ko.bindingHandlers.autocomplete = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            var freeText = options.freeText || defaultOptions.freeText;
            ...
            var dropdown = createElement('ul', {'class': 'knockout-autocomplete menu', 'data-bind': 'foreach: items, visible: items().length'});
            ...
            var suggestions = ko.computed(function () {
                ...
                if (queryText.length <= minLength) {
                    ...
                    var matches = ko.utils.arrayFilter(data, function (item) {
                        var label = item.toString().toLowerCase();
                        return label.indexOf(queryText) !== -1 &&
                            label !== queryText;
                    }).slice(0, maxItems);
            ...
            function freeTextSelect() {
                element.value = options.onSelect(element.value) || '';
                query('');
            }
            ...
            function select(e) {
                ...
                var hasSelection = i !== -1;
                if (hasSelection) {
                    element.value = options.onSelect(items[i].item) || '';
                    query('');
                    return true;
                } else if (freeText && element.value) {
                    freeTextSelect();
                    return true;
                }
            }
            ...
            ko.utils.registerEventHandler(element, "keypress", function (e) {
                if (freeText && separators.indexOf(String.fromCharCode(e.which)) !== -1) {
                    if (element.value) {
                        freeTextSelect();
                    }
