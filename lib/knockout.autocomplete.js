    ...
    ko.bindingHandlers.autocomplete = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            function freeTextSelect() {
                ...
                text = text && text.split(separators);
                element.value = options.onSelect(text) || '';
            ...
            function findItem(query) {
                var items = suggestions();
                for (var i = 0; i < items.length; i += 1) {
                    var label = format(items[i]).toLowerCase();
            ...
            ko.utils.registerEventHandler(element, "keydown", function (e) {
                ...
                //TODO take care of paste
                return true;
            });

            ko.utils.registerEventHandler(element, "keypress", function (e) {
                var isSeparator = separators && separators.test(String.fromCharCode(e.which));
                if (isSeparator && validate(element.value)) {
                    freeTextSelect();
                    if (e.preventDefault) e.preventDefault();
                    return false;
                }
